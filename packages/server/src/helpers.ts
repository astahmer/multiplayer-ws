...
export const makeRoom = ({ name, state }: { name: string; state?: ObjectLiteral }): LobbyRoom => ({
    name,
    clients: new Set(),
...
export const makeGameRoom = (name: string, config?: Partial<GameRoomConfig>): GameRoom => ({
    ...makeRoom({ name }),
